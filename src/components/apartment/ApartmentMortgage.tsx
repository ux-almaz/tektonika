                    ...
                    {downPresets.map((p) => (
                      <button
                        ...
                        className={`rounded-pill border px-4 py-1.5 text-sm transition-colors ${
                          down === p ? "bg-foreground text-background border-foreground" : "btn-interactive btn-outline border-border bg-transparent"
                        }`}
                      >
                        {p}%
                      </button>
                ...
                {banks.map((b, i) => {
                  ...
                  return (
                    <div key={i} className="grid grid-cols-[1fr_1fr_0.7fr_auto] gap-4 items-center py-5 border-b border-border last:border-b-0">
                      ...
                      <span>от {b.rate}%</span>
                      <button className="btn-interactive btn-outline rounded-pill border border-border bg-transparent px-5 py-2.5 text-xs font-medium uppercase tracking-wide whitespace-nowrap">
                        Подать заявку
                      </button>
                    </div>
                  );
